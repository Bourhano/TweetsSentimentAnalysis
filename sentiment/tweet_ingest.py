...
class StdOutListener(StreamListener):
    def on_data(self, data):
        ...
        producer.send(out_topic_name, json_["text"])
        return True
...
stream.filter(languages=["en"], track=["boris", "johnson"])

# api = API(auth)
#
# g = geocoder.osm('France')
#
# closest_loc = api.closest_trends(g.lat, g.lng)
# trends = stream.get_place_trends(closest_loc[0]['woeid'])
